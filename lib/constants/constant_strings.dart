class Strings {
  static String article_url = "https://api.realworld.io/api/articles";

  static String register_url = "https://api.realworld.io/api/users";
  static String login_url = "https://api.realworld.io/api/users/login";
  static String current_user_url = "https://api.realworld.io/api/user";
  static String create_article_url = "https://api.realworld.io/api/articles";
  static String setting_url = "https://api.realworld.io/api/user";
  static String favorited_url1 = "https://api.realworld.io/api/articles";
  // /{{slug}}/
  static String favorited_url2 = "favorite";
  static String deleteFav_url1 = "https://api.realworld.io/api/articles";
  // /{{slug}}/
  static String deleteFav_url2 = "favorite";
  static String likedArticle_url =
      "https://api.realworld.io/api/articles?favorited=";

  static String deleterArticle_url = "https://api.realworld.io/api/articles/";

  static String comment_url1 = "https://api.realworld.io/api/articles";
  // /{{slug}}/
  static String comment_url2 = "comments";
}
